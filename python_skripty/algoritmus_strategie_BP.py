...
cesta_skriptu = os.path.dirname(os.path.abspath(__file__))
cesta_root = os.path.dirname(cesta_skriptu)

cesta_data = os.path.join(cesta_root, 'data')
cesta_vystupy = os.path.join(cesta_root, 'vystupy')
vstupni_soubor = os.path.join(cesta_data, 'obchody.xlsx')
...
# Základ pro výpočet ročního výnosu = majetek k poslednímu obchodnímu dni PŘEDCHOZÍHO roku (INFO)
# Metoda: poslední den roku N-1 → poslední den roku N (stejná metodika jako benchmark PX index)
zaklad_pro_vynos = {
    'datum': '2004-01-05',      # Pro rok 2004: startovní kapitál (žádný předchozí rok)
    'majetek': startovni_kapital
}
...
for den in unikatni_dny:
    ...
    # -------------------------------------------------------
    # 1. ZACATEK – pouze informativní výpis, výnos se z něj nepočítá
    # -------------------------------------------------------
    ...
    if je_info or je_posledni_den:
        ...
        # Základ = majetek k poslednímu obchodnímu dni PŘEDCHOZÍHO roku (INFO roku N-1)
        # → stejná metodika jako PX index: 31.12.N-1 → 31.12.N
        majetek_zacatek = zaklad_pro_vynos['majetek']
        datum_zacatek   = zaklad_pro_vynos['datum']
        vynos_pct = round((majetek_konec - majetek_zacatek) / majetek_zacatek * 100, 2)

        rocni_prehled.append({
            'Rok': rok,
            'Datum_zacatek': datum_zacatek,
        ...
        # Aktuální INFO se stává základem pro výpočet výnosu příštího roku
        zaklad_pro_vynos = {
            'datum': pd.Timestamp(den).date(),
            'majetek': majetek_konec
        }
...
vystup_transakce = os.path.join(cesta_vystupy, "detailni_transakce.xlsx")
...
if rocni_prehled:
    ...
    vystup_vykonnost = os.path.join(cesta_vystupy, "vysledna_vykonnost.xlsx")
